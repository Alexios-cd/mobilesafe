...
/**
 * 主要功能：<br>
 * 1.根据packageName获取包信息  getPackageInfo<br>
 * 2.获取本应用的VersionCode，getVersionCode<br>
 * 3.判断是否是第三方软件isThirdPartyApp<br>
 * 4.读取指定路径下APK文件签名getJarSignature<br>
 * 5.根据公钥获取key，getPKBytes<br>
 * 6.计算签名时的hashcode算法hashCode<br>
 * 7.通过包名读取已安装APP数字签名getInstalledPackageSignature<br>
 * 8.获取指定路径的apk的资源getAPKResources<br>
 * 9.卸载应用<br>
 * 10.安装应用<br>
 * @author 傅文江
 * @2015-1-7 上午11:04:55
...
public class PackageUtils {
	...
	public static String getAppName()
	{
		try
		{
			...
			return UIUtils.getContext().getResources().getString(labelRes);
	...
	/**
	 * 智能安装应用，如果是系统应用或已经root，则静默安装，否则一般安装
	 * @param filePath 需要安装的应用路径
	 * @return
	 */
	public static final int install(String filePath) {
		if (PackageUtils.isSystemApplication()
				|| ShellUtils.checkRootPermission()) {
			return installSilent(UIUtils.getContext(), filePath);
		}
		return installNormal(filePath) ? INSTALL_SUCCEEDED
	 ...
	 * @return
	 */
	public static final int uninstall(String packageName) {
		if (PackageUtils.isSystemApplication()
				|| ShellUtils.checkRootPermission()) {
			return uninstallSilent(packageName);
		}
		return uninstallNormal(packageName) ? DELETE_SUCCEEDED
	 ...
	 */
	public static int uninstallSilent(String packageName, boolean isKeepData) {
		if (packageName == null || packageName.length() == 0) {
	...
	public static boolean isSystemApplication() {
		...
		return isSystemApplication(UIUtils.getContext().getPackageName());
	}
	...
	public static boolean isSystemApplication(String packageName) {
		...
		return isSystemApplication(UIUtils.getContext().getPackageManager(),
				packageName);
	}
	...
	public static Boolean isTopActivity(String packageName) {
		if (UIUtils.getContext() == null || StringUtils.isEmpty(packageName)) {
			return null;
		}

		UIUtils.getContext();
		ActivityManager activityManager = (ActivityManager) UIUtils
	...
	public static int getAppVersionCode() {
		if (UIUtils.getContext() != null) {
			...
			if (pm != null) {
				PackageInfo pi;
				try {
					pi = pm.getPackageInfo(UIUtils.getContext()
							.getPackageName(), 0);
					if (pi != null) {
